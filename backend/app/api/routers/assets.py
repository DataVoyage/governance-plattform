...
def _tool_aus(db: Session, tool: ToolObjekt) -> ToolAus:
...
def liste_tools(
    ...
    return [_tool_aus(db, t) for t in treffer]
...
def lege_tool_an(daten: ToolAnlegen, principal: AktuellerNutzer, db: DbSession) -> ToolAus:
    return _tool_aus(db, asset_service.lege_tool_an(db, principal, daten.model_dump()))
...
def tool_detail(tool_id: uuid.UUID, principal: AktuellerNutzer, db: DbSession) -> ToolAus:
    return _tool_aus(db, asset_service.hole_tool_sichtbar(db, principal, tool_id))
...
def aendere_tool(
    ...
    return _tool_aus(
        db, asset_service.aendere_tool(db, principal, tool, daten.model_dump(exclude_unset=True))
    )
...
def bestaetige_tool(tool_id: uuid.UUID, principal: AktuellerNutzer, db: DbSession) -> ToolAus:
    tool = asset_service.hole_tool_sichtbar(db, principal, tool_id)
    return _tool_aus(db, asset_service.bestaetige_tool(db, principal, tool))
...
def verknuepfe_prozess(
    ...
    return _tool_aus(db, asset_service.verknuepfe_tool_mit_prozess(db, principal, tool, prozess))
...
def loese_prozess(
    ...
    return _tool_aus(db, asset_service.loese_tool_von_prozess(db, principal, tool, prozess))
...
def attestiere_tool(
    ...
    """
    tool = asset_service.hole_tool_sichtbar(db, principal, tool_id)
    return _tool_aus(db, asset_service.attestiere(db, principal, tool, daten.model_dump()))
